...
def configure_lowmem_symbols(root_dir):
    ...
    if cache_path.exists():
        import pickle
        print(f'[lowmem] Loading symbols from cache: {cache_path}')
        with open(cache_path, 'rb') as f:
            cache_data = pickle.load(f)
        # Build a LazySymbolStore-compatible store from cache
        raw_symbols = cache_data['symbols']
        num_sources = len(cache_data.get('source_files', []))
        if num_sources == 0:
            num_sources = 2
        class CachedStore:
            """Minimal store compatible with LazyChromiumSymbols."""
            _raw = raw_symbols
            _elem_cache = {}
            ...
            def get(self, tag, name):
                ...
                if key in self._elem_cache:
                    return self._elem_cache[key]
                # Search in source order: chrome.dll (0) first, content.dll (1) second
                for src_idx in range(self._num_sources):
                    raw = self._raw.get((src_idx, tag, name))
                    if raw:
                        elem = ET.fromstring(raw)
                        self._elem_cache[key] = elem
                        return elem
        ...
        # --- Fallback: stream-parse XML (slow, ~4 min, but still low-memory) ---
        print('[lowmem] Cache not found, falling back to stream-parse XML...')
        print('[lowmem] Tip: Run "python3 symbol_cache.py" to build cache for faster startup.')
        symbol_files = [
    ...
    # Install stub chracer package (bypass __init__.py which would load full XML)
    ...
    # Patch get_type_size for common C++ types
